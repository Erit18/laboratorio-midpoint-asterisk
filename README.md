...
Este paso crea el rol que activa el aprovisionamiento automático de extensiones SIP. Cuando se asigna este rol a un usuario con `Telephone number`, midPoint llama automáticamente al endpoint del panel CDR, que configura la extensión en Asterisk sin intervención manual.
...
El output debe terminar con `Processed: 1, error: 0`. Si ves `error: 1` con el mensaje `No field 'expressionProfile'`, consulta el problema `n)` en la sección de problemas conocidos.

Verifica que el rol aparece en midPoint: `http://localhost:8080` → **Roles** → **All roles** — debe aparecer "AgenteCallCenter".

**Paso 7 — Importar la configuración del sistema (notifier de aprovisionamiento)**

Este paso configura el notifier Groovy en midPoint que dispara automáticamente el aprovisionamiento SIP cuando un usuario recibe el rol "AgenteCallCenter". **Sin este paso, el aprovisionamiento no será automático.**
...
**✅ Instalación completa.** A partir de este punto, cada vez que crees un usuario en midPoint con `Telephone number` y le asignes el rol "AgenteCallCenter", la extensión SIP se aprovisionará automáticamente en Asterisk. La contraseña SIP generada sigue el patrón `SIP` + número + `2026` (ej. extensión `1001` → `SIP10012026`).

> **⚠️ Nota sobre `sync_asterisk.sh`:** este script ya no es necesario para el flujo normal. Solo úsalo si necesitas agregar una extensión sin pasar por midPoint (pruebas rápidas). Las extensiones creadas con este script usan la contraseña que tú indiques, no el patrón automático.

**Si necesitas reiniciar todo desde cero**, borra los volúmenes primero y repite los pasos 1 a 7:
```bash
docker compose down -v
docker compose up -d
```
...
> Esta sección asume que ya completaste los 7 pasos de "Instalación desde cero" al menos una vez en esta máquina.

### 1. Verificar que la infraestructura esté activa

Los contenedores se levantan automáticamente al encender la VM. Verifica que estén corriendo:
```bash
docker compose ps
```
Confirma que `db`, `midpoint`, `asterisk` y `cdr-panel` estén `Up` y que `midpoint` diga `(healthy)`. Espera 3-5 minutos después de encender la VM antes de usar midPoint.

### 2. Flujo de Aprovisionamiento midPoint → Asterisk (Demo Principal)

Este es el flujo central del proyecto — midPoint actúa como fuente de la verdad y los agentes se aprovisionan automáticamente en Asterisk.

**Paso A — Crear un nuevo agente en midPoint:**
1. Abre `http://localhost:8080` y entra con `administrator` / `Callcenter2026!`
2. Ve a **Users → New user → Person**
...
4. Ve a **Assignments → Role** → agrega **AgenteCallCenter** → **Save**

**Paso B — Verificar el aprovisionamiento automático:**

Al hacer **Save**, midPoint ejecuta automáticamente el notifier Groovy que crea la extensión en Asterisk. Verifica en terminal:
```bash
docker exec callcenter-asterisk asterisk -rx "pjsip show endpoints" | grep 1001
```
Debe mostrar `Endpoint: 1001  Unavailable` — extensión lista para recibir registro SIP.

> **Contraseña SIP generada automáticamente:** `SIP` + Telephone number + `2026`
> Ejemplo: extensión `1001` → contraseña `SIP10012026`

**Paso C — Verificar en PostgreSQL:**
```bash
docker exec -it callcenter-db psql -U callcenter_user -d callcenter --pset pager=off \
  -c "SELECT nameorig, fullname, lifecyclestate FROM m_user WHERE nameorig = 'jperez';"
```

### 3. Aprovisionamiento manual (solo para emergencias)

Solo si necesitas agregar una extensión sin pasar por midPoint:
```bash
./sync_asterisk.sh 1001 miclave
```
> Las extensiones creadas con este script usan la contraseña que tú indiques, no el patrón automático.

### 4. Prueba de Concepto (Llamada y Evidencias)
1. Configura Zoiper con los datos del agente creado:
   - **Servidor:** IP de la VM (`ip addr show | grep "inet " | grep -v 127`)
   - **Usuario:** el Telephone number (ej. `1001`)
   - **Contraseña:** `SIP10012026`
   - **Puerto:** 5060 UDP
2. Realiza una llamada entre dos extensiones y contéstala.
3. Para mostrar verbosidad en vivo:
```bash
docker logs -f callcenter-midpoint 2>&1 | grep -i "SIP provision\|HTTP 200"
```

### 5. Panel CDR y Evidencias
Abre `http://localhost:8088` — muestra llamadas, métricas, extensiones activas en tiempo real y permite reproducir grabaciones con ▶.

### 6. Auditoría de Seguridad (ISO 27001)
En `http://localhost:8080` → reportes de auditoría → trazabilidad de accesos (control A.8.16 de ISO 27001).
...
**Si es la primera vez en esta máquina:** completa los 7 pasos de "Instalación desde cero" con tiempo de sobra — el Paso 4 puede tardar entre 50 minutos y 2 horas.

**⚠️ Red distinta a la de pruebas:** la IP cambia con cada red. Verifica la IP antes de configurar Zoiper (ver punto "d" de Problemas Conocidos).

1. `docker compose ps` → confirma que los 4 contenedores estén `Up` y que `midpoint` diga `(healthy)`. Espera 3-5 min si acabas de encender la VM.
2. **Demo midPoint → Asterisk (flujo automático):**
   - Abre `http://localhost:8080` → `administrator` / `Callcenter2026!`
   - **Users → New user → Person**
   - Llena: Name (único), Given name, Family name, **Telephone number = número de extensión**
   - **Assignments → Role → AgenteCallCenter → Save**
   - La extensión se crea automáticamente en Asterisk
   - Contraseña SIP: `SIP` + número + `2026` (ej. `1001` → `SIP10012026`)
   - Verifica: `docker exec callcenter-asterisk asterisk -rx "pjsip show endpoints" | grep NNNN`
3. Configura Zoiper: Servidor = IP de la VM, Usuario = Telephone number, Contraseña = `SIPnumero2026`, Puerto = 5060 UDP.
4. Haz una llamada de prueba y contéstala (genera CDR + grabación).
5. Abre `http://localhost:8088` → muestra métricas, extensiones en tiempo real y reproduce la grabación con ▶.
6. Muestra auditoría en `http://localhost:8080` → reportes de acceso.
7. Corre las pruebas unitarias:
```bash
python3 -m pytest tests/test_provision.py -v
```
Debe salir: `20 passed`.
